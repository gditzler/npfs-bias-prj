...
for nf = 1:numel(n_features)
  for ns = 1:numel(n_select)
    for nb = 1:numel(n_boots)
      try 
        disp(['loading ../mat/single_mim/experiment_scale_bootstraps_',method,'_nf',num2str(n_features(nf)),...
          '_nb',num2str(n_boots(nb)),'_ns',num2str(n_select(ns))]);
        load(['../mat/single_mim/experiment_scale_bootstraps_',method,'_nf',num2str(n_features(nf)),...
